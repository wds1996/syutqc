...
public class AssessAction extends BaseAction implements ModelDriven<Assess> {
	...
	public String add() throws IOException, TemplateException {
		...
		dao.addAssess(assess);
		return "toList";
	}
	public String list() throws IOException {
		response.setCharacterEncoding("UTF-8");
		AssessDao dao = new AssessDao();
		List<Assess> list = dao.findAll();
	...
	public String findAll() throws IOException {
		...
		List<Assess> list = dao.findAll();
		PageInfo<Assess> pageInfo = new PageInfo<>(list);
	...
	public String findById() throws IOException {
		response.setCharacterEncoding("UTF-8");
		AssessDao dao = new AssessDao();
		assess = dao.selectAssess(assess.getId());
		return "new";
	}
	...
	public String keywordSearch() throws IOException {
		...
		AssessDao dao = new AssessDao();
		List<Assess> list = dao.keywordSearch(keyword);
		String result = JSONArray.fromObject(list).toString();
		PrintWriter out = response.getWriter();
		out.print(result);
		out.flush();

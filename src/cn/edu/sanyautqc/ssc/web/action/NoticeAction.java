...
public class NoticeAction extends BaseAction implements ModelDriven<Notice> {
	...
	public String findById() throws IOException {
		response.setCharacterEncoding("UTF-8");
		NoticeDao dao = new NoticeDao();
		notice = dao.selectNotice(notice.getId());
		return "newnotice";
	}
	public String delete() {
		String id = request.getParameter("id");
		NoticeDao dao = new NoticeDao();
		Notice notice = dao.selectNotice(id);
		File file2 = new File(notice.getFileUrl());
		if (file2.exists())
			file2.delete();
		dao.deleteNotice(id);
		return "toList";
	...
	public String keywordSearch() throws IOException {
		...
		NoticeDao dao = new NoticeDao();
		List<Notice> list = dao.keywordSearch(keyword);
		String result = JSONArray.fromObject(list).toString();
		PrintWriter out = response.getWriter();
		out.print(result);
		out.flush();

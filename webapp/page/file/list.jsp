	...
	<script type="text/javascript">
 								...
 								for (var i = 0; i < array.length; i++) {
 									$(".searchList").append(			    		
										'<tr class="item">' +
 						    				...
 						    				'<td>' + array[i].time +'</td>' +
 						    				'<td>' +
 												'<a class="layui-btn layui-btn-mini" href="<%=basePath%>assess_toUpdate.action?id=' + array[i].id + '"><i class="iconfont icon-edit"></i> 编辑</a>' + 
						...
						for (var i = 0; i < array.length; i++) {
							$(".mainList").append(			    		
									'<tr>' +
					    				...
					    				'<td>' + array[i].time +'</td>' +
					    				'<td>' +
											'<a class="layui-btn layui-btn-mini" href="<%=basePath%>file_toUpdate.action?id=' + array[i].id + '"><i class="iconfont icon-edit"></i> 编辑</a>' + 
											'<a class="layui-btn layui-btn-danger layui-btn-mini" href="javascript:void(0)" onclick="del(\'' + array[i].id + '\')"><i class="layui-icon">&#xe640;</i> 删除</a>' +
				        				'</td>' +
					        		'</tr>'
				   	 		)
		...
		function del(id) {
			if(confirm("确认删除吗?") == true){    
				location.href="<%=basePath%>file_delete.action?id="+id;
			} 
		}
	</script>
</head>
<body class="childrenBody">
	<blockquote class="layui-elem-quote news_search">
		...
		<form method="post" action="<%=basePath%>file_keywordSearch.action" class="searchForm">
		    <div class="layui-input-inline">
		    	<input type="text" name="keyword" placeholder="请输入关键字" class="layui-input search_input keyword">
		    </div>  
		</form> 
		</div>
		<div class="layui-inline">
		...
		<div class="layui-inline">
			<a class="layui-btn layui-btn-normal newsAdd_btn" href="<%=basePath%>page/file/add.jsp">添加文章</a>
		</div>
	</blockquote>

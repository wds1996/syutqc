...
<body class="childrenBody">
	<div class="closeBtn"><a href="<%=basePath%>page/process/list.jsp" target="_self"><i class="layui-icon">&#x1006;</i></a></div>
	<form class="layui-form" action="<%=basePath%>process_update.action" method="post">
		<input name="id" type="text" hidden="hidden" value="${process.id}">
		<div class="layui-form-item">
			<label class="layui-form-label">标题</label>
			<div class="layui-input-block">
				<input name="name" type="text" value="${process.name}" class="layui-input newsName" lay-verify="required" placeholder="请输入文章标题">
			</div>
		</div>
		<div class="layui-form-item">
			<div class="layui-inline">		
				...
				<div class="layui-input-inline">
					<input name="author" type="text" value="${process.author}" class="layui-input newsAuthor" lay-verify="required" placeholder="请输入文章作者">
				</div>
			</div>
			<div class="layui-inline">		
				<label class="layui-form-label">发布时间</label>
				<div class="layui-input-inline">
					<input type="text" class="layui-input" id="time" name="time" value="${process.time}" autocomplete="off">
				</div>
			</div>
			<div class="layui-inline">
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<script type="text/plain" id="myEditor" name="content" style="width:1000px;height:240px;">${process.content}</script>
			</div>
		</div>
		<div class="layui-form-item">
			<div class="layui-input-block">
  ...
  layui.use('upload', function(){
	  ...
	  var uploadInst = upload.render({
	    elem: '#fileUpload' 
	    ,url: '<%=basePath%>process_uploadFile.action'
	    ,accept: 'file'
	    ,size: '10240'
	    ,done: function(res){
	    	if(res.code > 0){
  ...
  function del(name) {
		if(confirm("确认删除吗?") == true){
			...
			$.ajax({
		  		type: 'post',
		  		url: '<%=basePath%>process_delFile.action',
		  		traditional: true,
		  		data: json_str,
		  		success: function() {
		  			$(".delFile").empty();
  ...
  $(function() {
		if (!jQuery.isEmptyObject('${process.fileName}')) {
			$(".fileName").val("${process.fileName}");
	    	$(".fileUrl").val("${process.fileName}");
			$(".delFile").append('<span style="color: green;">' + '${process.fileName}' + '</span>&nbsp;&nbsp;&nbsp;'+
					 '<a class="layui-btn layui-btn-danger layui-btn-mini" href="javascript:void(0)" onclick="del(\'' + '${process.fileName}' + '\')"><i class="layui-icon">&#xe640;</i> 删除</a>');
		}
})
	</script>
</body>

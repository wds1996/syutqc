...
<head>
	...
	<script type="text/javascript">
 								...
 								for (var i = 0; i < array.length; i++) {
 									$(".searchList").append(			    		
										'<tr class="item">' +
 						    				...
 						    				'<td>' + array[i].time +'</td>' +
 						    				'<td>' +
												'<a class="layui-btn layui-btn-mini" href="<%=basePath%>suggest_detail.action?id=' + array[i].id + '"><i class="iconfont icon-edit"></i> 查看</a>' + 
		...
		function initTable() {
			$.ajax({
				type: 'post',
				url: '<%=basePath%>suggest_getList.action',
				traditional: true,
				dataType: 'json',
				success: function(array) {
					if (jQuery.isEmptyObject(array)) {
		...
		function del(id) {
			if(confirm("确认删除吗?") == true){    
				location.href="<%=basePath%>suggest_delete.action?id="+id;
			} 
		}
	</script>
</head>
<body class="childrenBody">
	<blockquote class="layui-elem-quote news_search">
		...
		<form method="post" action="<%=basePath%>suggest_keywordSearch.action" class="searchForm">

	...
	<script type="text/javascript">
 								...
 								for (var i = 0; i < array.length; i++) {
 									$(".searchList").append(			    		
										'<tr class="item">' +
 						    				...
 						    				'<td>' + array[i].time +'</td>' +
 						    				'<td>' +
 												'<a class="layui-btn layui-btn-mini" href="<%=basePath%>work_toUpdate.action?id=' + array[i].id + '"><i class="iconfont icon-edit"></i> 编辑</a>' + 
		...
		function del(id) {
			if(confirm("确认删除吗?") == true){    
				location.href="<%=basePath%>work_delete.action?id="+id;
			} 
		}
	</script>
</head>
<body class="childrenBody">
	<blockquote class="layui-elem-quote news_search">
		...
		<form method="post" action="<%=basePath%>work_keywordSearch.action" class="searchForm">
		...
		<div class="layui-inline">
			<a class="layui-btn layui-btn-normal newsAdd_btn" href="<%=basePath%>page/work/add.jsp">添加文章</a>
		</div>
	</blockquote>
